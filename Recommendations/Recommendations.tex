...
We would like to extend our heartfelt thanks to each of you for your time and effort in reviewing the document. Your input has been invaluable in refining and enhancing the quality of the work. For your convenience, all changes made to the document are highlighted in \changes{red color}. Below, you will find our point-by-point responses to each of your comments.\\
...
\textbf{Thank you for taking the time to review the document. We appreciate your approval}\\
...
\textbf{We sincerely appreciate all the valuable comments and recommendations you have provided.}\\

\point{...cerebrales en los sistemas MI-BCI \genaro{es} recolectada usando electroencefalografía...
}

\reply{We have change the grammar mistake as folllows:
    ...cerebrales en los sistemas MI-BCI \changes{son} recolectada usando electroencefalografía...
}

\point{
    El estudio aporta contribuciones hacia un \genaro{FC single-trial} en los sistemas MI-BCI 
}

\reply{We have rewritten the sentence as follows:
    El estudio aporta contribuciones hacia \changes{las FC single-trial} en los sistemas MI-BCI
}
...
\point{
    Header "7.1 Cross-Information Potential Regularization 7. Interpretable Regularized Kernel Cross-Spectral FC Network" has word overlapping.
}
\reply{
...
\point{This chapter is missing a description of the optimization problem. The entropy is maximized with respect to what variables? The full optimization with the trade-off with respect to the cross-entropy should be detailed.}

\reply{
    Thank you for the recommendation. We agree that a optimization problems is missing, so it is included as follows:
    \changes{Since the main objective is to enhance the interpretability while maintaining the results obtained in \cref{chapter_2}, we include the $H(\chi; \mathbf{w}_f, \sigma)$ into the optimization problem as follows: 
    \begin{equation}
      \Theta^{*} = \underset{\Theta}{\arg\,\min} \quad \promeddd{r}{\mathcal{L}(\ve{y}_r,\hat{\ve{y}}_r|\Theta) - H(\tilde{\mat{P}_r}; \mathbf{w}_f, \sigma); \forall r\in\{1,2,\dots,R\}},
    \end{equation}
...
\point{Legend is missing in 7-3 (although the colors are described in the text, it should be in the figure and/or caption). }

\reply{
    Thank you for the recommendation, we include the legend to the figure and describe each color in the caption.
...
\point{It seems some of the future work in the conclusion directly contradicts what was said earlier about avoiding complexity of graph neural networks. Perhaps those earlier comments could be tempered.}

\reply{
    Thank you for the valuable comment, we agree and change the paragraph as:
...
\point{Units like mV, V, and Hz are not italics. Nor should reference names be italics. "Table X", "Figure X", "Section X", etc. should be capitalized and not abbreviated like in Chapter 6, but consistent throughout.  }

\reply{
    Thank you for the recommendation we removed all italics refering to Units and names. Also "Table X", "Figure X", "Section X" was changed as suggested.
}
